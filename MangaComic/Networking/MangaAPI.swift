...
class MangaAPI {
    ...
    func getPagesMangaWithNumberChapter(endPoint: String, completion: @escaping ([MangaPage]) -> Void) {
        let url = "http://localhost:3000/manga/chapter/\(endPoint)" // Thay bằng URL API thực tế
    ...
    func getMangaDetail(endPoint: String, completion: @escaping (MangaDetail) -> Void) {
        let url = "http://localhost:3000/manga/detail/\(endPoint)" // Thay bằng URL API thực tế
